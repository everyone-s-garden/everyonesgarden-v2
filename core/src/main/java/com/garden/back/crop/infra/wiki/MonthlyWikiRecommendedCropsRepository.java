...
public class MonthlyWikiRecommendedCropsRepository {
    ...
    public void setUp() {
        ...
        wikiRecommendedCrops.add(new WikiRecommendedCrop("마늘", "마늘은 한파에 약하고 가뭄에도 취약하여 전세계적으로 온대와 아열대 지역에서 재배한다.[1] 더위에도 약하여 한지형의 경우 6월 하순경에는 지상부가 마르고, 약 3개월간 휴면기에 들어간다. 파종 후 저온기를 경과하여야 비늘줄기의 비대가 촉진되므로 한지형은 대개 10월 하순·11월 상순에 파종하는 데, 이보다 빨리 파종하여 연내에 지상부가 자라날 경우 추위에 약하게 된다. 봄에 파종할 경우에는 해동되자마자 파종해야만 비늘줄기가 알차게 성숙된다. 파종시에 흙덮기(복토)를 얇게 하면 솟아 나와서 동해를 입으므로 약 2-3cm가량 흙을 덮고 그 위에 두엄을 덮어서 보온하고, 이듬해 3월 중·하순에 벗긴다.[7] 난지형은 9월쯤 파종하여 다음해 5-6월에 수확한다.[15]", List.of(2), "https://ko.wikipedia.org/wiki/%EB%A7%88%EB%8A%98"));
