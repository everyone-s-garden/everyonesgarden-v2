...
class PostRestDocsTest extends RestDocsSupport {
    ...
    void findAllPosts() throws Exception {
        FindAllPostsResponse response = new FindAllPostsResponse(List.of(
        ...
        mockMvc.perform(get("/v1/posts")
            ...
            .andDo(document("get-all-posts",
                ...
                responseFields(
                    ...
                    fieldWithPath("postInfos[].userInfo").type(OBJECT).description("글쓴이 정보"),
                    fieldWithPath("postInfos[].userInfo.userId").type(NUMBER).description("글쓴이 정보"),
        ...
        mockMvc.perform(get("/v1/posts/{postId}", 1L)
            ...
            .andDo(document("get-details-posts",
                ...
                responseFields(
                    ...
                    fieldWithPath("userInfo").type(OBJECT).description("글쓴이 정보"),
                    fieldWithPath("userInfo.userId").type(NUMBER).description("글쓴이 정보"),
    ...
    void findPostsComments() throws Exception {
        FindPostsAllCommentResponse response = new FindPostsAllCommentResponse(
            ...
            false,
            List.of(
                new FindPostsAllCommentResponse.CommentInfo(
                    2L,
                    1L,
                    0L,
                    "대댓글",
        ...
        mockMvc.perform(get("/v1/posts/{postId}/comments", 1L)
            ...
            .andDo(document("get-posts-comments",
                ...
                responseFields(
                    fieldWithPath("mainComment").type(ARRAY).description("부모 댓글 정보 목록"),
                    fieldWithPath("mainComment[].commentId").type(NUMBER).description("댓글 ID"),
                    fieldWithPath("mainComment[].likeCount").type(NUMBER).description("좋아요 수"),
                    fieldWithPath("mainComment[].content").type(STRING).description("댓글 내용"),
                    fieldWithPath("mainComment[].userInfo").type(OBJECT).description("글쓴이 정보"),
                    fieldWithPath("mainComment[].userInfo.userId").type(NUMBER).description("글쓴이 정보"),
                    ...
                    fieldWithPath("mainComment[].subComments[].parentId").type(NUMBER).description("부모 댓글의 ID"),
                    fieldWithPath("mainComment[].subComments[].commentId").type(NUMBER).description("좋아요 수"),
                    fieldWithPath("mainComment[].subComments[].likeCount").type(NUMBER).description("자식 댓글 정보"),
                    fieldWithPath("mainComment[].subComments[].content").type(STRING).description("댓글 내용"),
                    fieldWithPath("mainComment[].subComments[].userInfo").type(OBJECT).description("글쓴이 정보"),
                    fieldWithPath("mainComment[].subComments[].userInfo.userId").type(NUMBER).description("글쓴이 정보"),
    ...
    void findPopularPosts() throws Exception {
        FindAllPopularPostsResponse response = new FindAllPopularPostsResponse(
            List.of(
